...
if __name__=="__main__":
    ...
    data_real = np.loadtxt(out_dir/ f"data_syn_all_res{res}m.txt")#[mask_dome]
    unc_real = np.loadtxt(out_dir/ f"unc_syn_all_res{res}m.txt")#[mask_dome]
    ...
    filename= Path.home()/"QGis_Elements/dome/contours_100m.shp"
    sf = shapefile.Reader(str(filename))
    shps = sf.shapes()
    ...
    FNORD = np.array([642958.52, 1774500.03])
    NAP = np.array([642994.49, 1774232.5])
    #isocurves altitudes
    d_l = { 0:{"z": 1200, "xy": np.array([642690, 1773850]), "r": -20},
            1:{"z":1300, "xy": np.array([642580, 1774343]), "r": 80},  
            2:{"z":1300, "xy": np.array([643365+50, 1773912+70]), "r": 30},  
            3:{"z":1400, "xy": np.array([643136-20, 1774437-50]), "r": -40}, 
        }
    ...
    for ix_rho0 in range(nrho): #
        ...
        fig_best_rho.subplots_adjust(left=0.08, bottom=0.1, right=0.87, top=0.95, wspace=0.15, hspace=0.15)
        ...
        fig_best_strength.subplots_adjust(left=0.08, bottom=0.1, right=0.87, top=0.95, wspace=0.15, hspace=0.15)
        ...
        fig_std.subplots_adjust(left=0.08, bottom=0.1, right=0.87, top=0.95, wspace=0.15, hspace=0.15)
        ...
        for ix_z, zslice in enumerate(lzslice):
            ...
            print(len(arr_rho_post[0,0,0]), len(mask_slice))
            #exit()
            ...
            for i in range(nrow):
                ...
                for j in range(ncol):
                    ...
                    ax.tick_params(which="both", bottom=True, top=True, left=True, right=True, labelsize=fontsize)
                    if i==0: 
            ...
            sig_sel, lc_sel = 0.3, 200
            phi_d = misfit_d[ix_rho0]
            #a=abs(phi_d-1)
            #ij_d = np.unravel_index(np.argmin(a, axis=None), a.shape) #np.argmin(abs(misfit_d[ix_rho0]-1), axis=1)
            
            try : 
                ij_d = (np.unravel_index(np.argmin(abs(mat_sigma-sig_sel)), mat_sigma.shape)[0], np.unravel_index(np.argmin(abs(mat_length-lc_sel)[1]), mat_length.shape)[1])
            except : 
                ij_d = np.unravel_index(np.argmin(abs(mat_sigma-sig_sel)), mat_sigma.shape)
            ...
            try : 
                ij_m = (np.unravel_index(np.argmin(abs(mat_sigma-sig_sel)), mat_sigma.shape)[0], np.unravel_index(np.argmin(abs(mat_length-lc_sel)[1]), mat_length.shape)[1])
            except : 
                ij_m = np.unravel_index(np.argmin(abs(mat_sigma-sig_sel)), mat_sigma.shape)
            ...
            cb = fig.colorbar(ScalarMappable(norm=cm.Normalize(vmin=rho_min, vmax=rho_max), cmap=cmap_rho), cax=cax)
            ...
            arg_col_true =  [np.argmin(abs(range_val_rho-v))for v in rho_vox_true[mask_slice]]
            color_vox_true = color_scale_rho[arg_col_true]
            ...
            for axb in [ax_best_rho[zslice], ax_best_strength[zslice], ax_std[zslice]]:
                ...
                if axb == ax_best_rho[zslice]:
                    ...
                    for l, v in enumerate(verts):
                        ...
                        pci = PolyCollection([v[:,:-1]]-np.array([vc_x, vc_y]),  
                                                cmap=cmap_rho,
                                                #alpha=0.3,
                                                facecolors=color_vox_true[l], #np.repeat(color_vox,axis=0),
                                                edgecolors=np.clip(color_vox_true[l] - 0.5, 0, 1),  # brighter 
                                                norm=colors.Normalize(clip=True),
                                                linewidths=0.3)
                        pci.set_clim(vmin_r, vmax_r)
                        axbin.add_collection(pci)
                    
                    # axbin.set_xlim([xrange[0]+200,xrange[1]-200])
                    # axbin.set_ylim([yrange[0]+200,yrange[1]-200])
                    axbin.set_xlim([-0.6*dx, 0.6*dx])
                    axbin.set_ylim([-0.6*dx, 0.6*dx])
                    #axbin.axis('off')
                    axbin.tick_params(
                ...
                anchored_text = AnchoredText(f"z = {zslice:.0f} m a.s.l", loc="upper left", frameon=True, prop=dict(fontsize=fontsize+2))
                axb.add_artist(anchored_text)
                #ipan = AnchoredText(f"{l_ipan[ix_z]}", loc="upper right", frameon=True, prop=dict(fontsize=fontsize+2))
                #axb.add_artist(ipan)
        ...
        for ax in [ax_best_rho, ax_best_strength, ax_std]:
            ...
            for i, shape in enumerate(shps):
                ...
                for z in lzslice: 
                    ax[z].plot(lx-vc_x, ly-vc_y, linewidth=1., marker=".",  markersize=0., color="black") 
                    #ax[z].scatter(TAR[0]-vc_x, TAR[1]-vc_y, s=30, marker="D", color="deepskyblue") #"cyan"
                    #ax[z].scatter(CSS[0]-vc_x, CSS[1]-vc_y, s=30, marker="D", color="deepskyblue") #"cyan"
                    #ax[z].scatter(NAP[0]-vc_x, NAP[1]-vc_y, s=30, marker="D", color="deepskyblue") #"cyan"
            ...
            ax1 = ax[lzslice[-1]] 
            ax2 = ax[lzslice[1]]     
            ax[lzslice[-2]].set_xlabel("X [m]", fontsize=fontsize+2)
            ax[lzslice[-1]].set_xlabel("X [m]", fontsize=fontsize+2)
            ax[lzslice[0]].set_ylabel("Y [m]", fontsize=fontsize+2)
            ax[lzslice[-2]].set_ylabel("Y [m]", fontsize=fontsize+2)
            if ax == ax_best_rho : 
                ax[lzslice[0]].set_title(best_par_d)
            ...
            if ax == ax_best_strength : 
                ...
                cb.set_label(label="unixial compressive strength [MPa]", size=fontsize+2)#, labelpad=1.)
            
            if ax == ax_std : 
                cax = fig_std.add_axes([ax1.get_position().x1+0.02,ax1.get_position().y0,0.02,ax2.get_position().y0])
                cb = fig_std.colorbar(pcstd,  cax=cax)
                locator = MultipleLocator(4e-2)
                cb.ax.yaxis.set_major_locator(locator)
                cb.set_label(label="standard deviation $\\~{\\sigma}$ [g.cm$^{-3}$]", size=fontsize+2)#, labelpad=1.)
            
                
            cb.ax.tick_params(which="both",labelsize=fontsize+2)

        
        fout = out_dir / f"best_misfit_data_rho_post_rho0{rho0}_res{res}m_xy"
        ...
        fout = out_dir / f"best_misfit_strength_rho0{rho0}_res{res}m_xy"
        fig_best_strength.savefig(f"{str(fout)}.png", dpi=300, transparent=True) 
        print(f"save {fout}.png")
        
        
        fout = out_dir / f"std_dev_rho0{rho0}_res{res}m_xy"
