...
#    xslice = vc_x
    ...
    for yslice in [vc_y, vc_y-50, vc_y-100]:
    
        xsmin, xsmax = np.nanmin(dome.barycenter[:,0]), np.nanmax(dome.barycenter[:,0])
        dx = 250
        ysmin, ysmax = vc_x - dx,  vc_x + dx
        ####Define slice mask
        mask_slice =  ((yslice - 3*res) <= dome.barycenter[:,1]) & (dome.barycenter[:,1] <= (yslice ))
        mask_slice = mask_slice & (xsmin <= dome.barycenter[:,0]) & (dome.barycenter[:,0] <= xsmax )
        ...
        for ix_rho0 in range(nrho): #
            ...
            for l, (vd, vu) in enumerate(zip(ilos_xyzdown, ilos_xyzup)):
               ...
               # v= [vd[0,1:], vd[2,1:], vu[1,1:], vu[0,1:]]#-np.array([vc_y, 0])
                v = [ [vd[2,0], vd[2,-1]] ,[vd[0,0], vd[0,-1]], [vu[0,0], vu[0,-1]],[vu[2,0], vu[2,-1]]]-np.array([vc_x, 0])   
            ...
            for a in [axr, axs, ax_std] : 
                ...
                a.set_xlabel("Easting X [m]")#, fontsize=fontsize)
               
                # m = (vc_x+dx > xa1) & (xa1 > vc_x-dx)
                # #pointsa1 = np.vstack((xa1[m]-vc_x,za1[m])).T
                # a.scatter(xa1-vc_x, za1, color="orange", marker="o",  s=40., alpha=0.5 )
                # a.scatter(xa2-vc_x, za2, color="red", marker="o",  s=40., alpha=0.5 )

            
                      
                anchored_text = AnchoredText(f"Y = {yslice-vc_y:.0f} m ", loc="upper left", frameon=True, prop=dict(fontsize='x-large'))
                ...
                a.scatter(TAR[0]-vc_x, TAR[-1],**kwargs) #"cyan"
                a.annotate(f"TAR", xy =(TAR[0]-vc_x, TAR[-1]+10), fontsize='x-large',
                             xycoords='data', annotation_clip=False ,alpha=0.8,fontweight='normal')  
                a.scatter(CSS[0]-vc_x, CSS[-1],**kwargs) #"cyan"
                a.annotate(f"CSS", xy =(CSS[0]-vc_x, CSS[-1]+10), fontsize='x-large',
                             xycoords='data', annotation_clip=False , alpha=0.8,fontweight='normal')  
            ...
            fout = out_dir / f"best_misfit_data_rho_post_rho0{rho0}_res{res}m_y{int(yslice)}_xz"
            fig_best_rho.savefig(f"{str(fout)}.png", transparent=True )
            print(f"save {fout}.png")
            fout = out_dir / f"best_misfit_strength_rho0{rho0}_res{res}m_y{int(yslice)}_xz"
            fig_best_strength.savefig(f"{str(fout)}.png", transparent=True)    
            print(f"save {fout}.png")
            
            fout = out_dir / f"std_dev_rho0{rho0}_res{res}m_y{int(yslice)}_xz"
